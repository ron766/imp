	...
	var $thm = {
		...
		ads:function() {
            let ads = {
                ...
                article: {
                    desktop: ["gpt-overlay", "gpt-skin", "topleaderboard", "ticker", "inread", "topimu","ciu","imu2","imu3", "imu4"],
		...
		setDeviceClassAdSizes:function(width) {
			try {
				...
				if (width >= this.deviceBreakpoints.wide) { // wide = 970
					...
					IDG.GPT.addSlot("topimu", [[300,600],[300,250]]); //updated per IDGMPM-15087 to include 300x600 even on slideshows for tablet/desktop
					if (!IDG.GPT.isSlideshow) {
